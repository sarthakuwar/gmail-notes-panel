...
// from the Polar webhook). This table is a thin cache of the payment
// provider's own subscription status — see supabase/schema.sql for why
// there's no independent trial/renewal clock here.
...
export async function getSubscription(ownerEmail: string): Promise<SubscriptionRow> {
  const { data } = await supabase
    .from("subscriptions")
    .select("owner_email, billing_customer_id, billing_subscription_id, status, current_period_end")
